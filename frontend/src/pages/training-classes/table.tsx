...
export default function TrainingClassesTable() {
  ...
  /* ---------- approve / reject ---------- */
  const [approveTarget, setApproveTarget] = useState<TrainingClass | null>(
    null,
  );
  ...
  const columns = useMemo(
    () =>
      getColumns({
        onNavigate: (row) =>
          navigate(`/classes/${encodeRouteId("classes", row.id)}`, {
            state: { trainingClass: row },
          }),
  ...
  const handleApprove = async (reason: string) => {
    ...
    try {
      ...
      await queryClient.invalidateQueries({
        queryKey: trainingClassKeys.detail(approveTarget.id),
      });
  ...
  const handleReject = async (reason: string) => {
    ...
    try {
      ...
      await invalidateAll();
      await queryClient.invalidateQueries({
        queryKey: trainingClassKeys.detail(rejectTarget.id),
      });
