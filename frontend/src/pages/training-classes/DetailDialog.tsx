    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="sm:max-w-[600px] p-0 overflow-hidden gap-0">
        ...
        <div className="p-6 max-h-[70vh] overflow-y-auto space-y-5">
          <DetailRow icon={GraduationCap} label="Class Name" value={trainingClass.className} />

          <DetailRow icon={Hash} label="Class Code" value={trainingClass.classCode} />

          <DetailRow icon={BookOpen} label="Semester" value={trainingClass.semesterName} />

          <div className="grid grid-cols-2 gap-5">
            <DetailRow icon={User} label="Creator" value={trainingClass.creatorName} />
            <DetailRow icon={User} label="Approver" value={trainingClass.approverName} />
          </div>
          ...
          <div className="grid grid-cols-2 gap-5">
            <DetailRow
              icon={Calendar}
              label="Start Date"
              value={trainingClass.startDate ? dayjs(trainingClass.startDate).format("DD-MM-YYYY") : undefined}
            />
            <DetailRow
              icon={Calendar}
              label="End Date"
              value={trainingClass.endDate ? dayjs(trainingClass.endDate).format("DD-MM-YYYY") : undefined}
            />
          </div>

          {trainingClass.description && (
            <DetailRow icon={FileText} label="Description" value={trainingClass.description} />
          )}

          <DetailRow icon={ToggleLeft} label="Status" value={trainingClass.isActive} isBadge />
        </div>
