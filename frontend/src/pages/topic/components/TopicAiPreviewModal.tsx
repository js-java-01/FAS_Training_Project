...
import {
  ...
  type TopicAiPreviewLessonResponse,
...
interface TopicAiPreviewModalProps {
  open: boolean;
  onClose: () => void;
  onApplied: () => void;
  topicId: string;
...
export function TopicAiPreviewModal({
  open,
  onClose,
  onApplied,
  topicId,
  topicName,
  topicCode,
}: TopicAiPreviewModalProps) {
  const [lessons, setLessons] = useState<TopicAiPreviewLessonResponse[]>([]);
  ...
  const fetchPreview = async () => {
    try {
      setLoading(true);
      const data = await topicBatchOutlineApi.generateAiPreview(topicId);
      setLessons(data);
  ...
  const handleApply = async () => {
    try {
      setApplying(true);
      await topicBatchOutlineApi.applyAiPreview(topicId, { lessons });
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    <h2 className="text-xl font-bold text-gray-900 mb-1">
                      {topicName ?? "Topic Outline"}
                    </h2>
                    {topicCode && (
                      <p className="text-sm text-gray-500 mb-1">{topicCode}</p>
                    )}
                  </div>
