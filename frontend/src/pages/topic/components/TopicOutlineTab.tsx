...
import { topicApi, type TopicLesson } from "@/api/topicApi";
...
const typeLabelMap: Map<string, string> = new Map(SESSION_TYPE_OPTIONS.map((o) => [o.value, o.label]));
const inputCls =
...
export function TopicOutlineTab({ topicId, isEditMode }: Props) {
  ...
  /* ── sessions per lesson ── */
  const [sessionsMap, setSessionsMap]             = useState<Record<string, SessionResponse[]>>({});
  ...
  const [activeSessionLessonId, setActiveSessionLessonId] = useState<string | null>(null);
  ...
  const [sessionForm, setSessionForm]     = useState({
    topic: "", type: "", studentTasks: "", sessionOrder: "", duration: "",
  });
  ...
  const fetchSessions = useCallback(async (lessonId: string) => {
    ...
    try {
      const data = await sessionService.getSessionsByLesson(lessonId);
  ...
  const openCreateSession = (lessonId: string) => {
    setActiveSessionLessonId(lessonId);
    const existing = sessionsMap[lessonId] ?? [];
    setSessionForm({
      topic: "", type: "", studentTasks: "",
  ...
  const openEditSession = (session: SessionResponse) => {
    ...
    setActiveSessionLessonId(session.lessonId);
    setSessionForm({
      topic:        session.topic ?? "",
      type:         session.type  ?? "",
  ...
  const handleSaveSession = async () => {
    if (!sessionForm.topic.trim()) {
      setSessionFormError("Content (topic) is required.");
      return;
    }
    ...
    const lessonId = activeSessionLessonId ?? editSession?.lessonId ?? "";
    if (!lessonId) { setSessionFormError("Lesson context is missing."); return; }

    const payload: SessionRequest = {
      lessonId,
      topic:        sessionForm.topic.trim(),
      type:         sessionForm.type || null,
      studentTasks: sessionForm.studentTasks.trim() || null,
    ...
    try {
      if (drawerMode === "edit-session" && editSession) {
        await sessionService.updateSession(editSession.id, payload);
                    ...
                    {sessionsLoading ? (
                      ...
                      <div className="overflow-hidden rounded-lg border border-gray-100">
                        ...
                        {[...sessions]
                          ...
                          .map((session) => {
                            const typeLabel = session.type ? (typeLabelMap.get(session.type) ?? session.type) : null;
                            const typeColor = session.type ? (TYPE_COLORS[session.type] ?? "bg-gray-100 text-gray-600") : "";
                            return (
                              <div
                                key={session.id}
                              ...
                              >
                                ...
                                <span className="truncate text-gray-800 pr-3" title={session.topic}>
                                  {session.topic || "—"}
                                </span>
                                <span>
                                  {typeLabel
        ...
        <SheetContent side="right" className="w-[480px] sm:w-[540px] flex flex-col p-0">
          <SheetHeader className="border-b px-6 py-4 shrink-0">
            <div className="flex items-center justify-between">
              <div>
                <SheetTitle className="text-base font-semibold">
                  {drawerMode === "edit-session" ? "Edit Session" : "Add Session"}
                </SheetTitle>
                <SheetDescription className="text-xs text-gray-400 mt-0.5">
                  {drawerMode === "edit-session" ? "Update session details" : "Add a new session to this lesson"}
                </SheetDescription>
              </div>
              <div className="flex items-center gap-2">
          ...
          <div className="flex-1 overflow-y-auto px-6 py-6 space-y-5">
            ...
            <div className="space-y-1.5">
              ...
              <textarea
                ...
                onChange={(e) => { setSessionForm((p) => ({ ...p, topic: e.target.value })); if (sessionFormError) setSessionFormError(null); }}
                placeholder="Enter session content / topic"
                className={inputCls + " resize-none"}
              />
            ...
            {/* Student Tasks */}
            <div className="space-y-1.5">
              <label className="text-sm font-medium text-gray-700">Student Tasks</label>
              <textarea
                rows={3}
                value={sessionForm.studentTasks}
                onChange={(e) => setSessionForm((p) => ({ ...p, studentTasks: e.target.value }))}
                placeholder="Describe student tasks (optional)"
                className={inputCls + " resize-none"}
              />
            </div>
          </div>
