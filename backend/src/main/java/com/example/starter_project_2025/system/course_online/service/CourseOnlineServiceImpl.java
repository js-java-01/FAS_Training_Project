...
public class CourseOnlineServiceImpl implements CourseOnlineService {
    ...
    private final CourseOnlineMapper mapper;
    private final UserService userService;
    private final CourseAssessmentSchemeOnlineService courseAssessmentSchemeService;
    private final CourseObjectiveOnlineService courseObjectiveService;
    private final BatchOutlineOnlineService batchOutlineService;

    @Override
    @Transactional
    public CourseOnlineResponse create(CourseCreateOnlineRequest req) {
        CourseOnline course = mapper.toEntity(req);
        course.setCreator(userService.getCurrentUser());
        CourseOnline saved = courseRepository.save(course);

        if (req.getTopicId() != null) {
            UUID courseId = saved.getId();
            UUID topicId = req.getTopicId();
            courseAssessmentSchemeService.cloneFromTopic(topicId, courseId);
            courseObjectiveService.cloneFromTopic(courseId, topicId);
    ...
    @Override
    public CourseOnlineResponse update(UUID id, CourseUpdateOnlineRequest req) {
    ...
    @Override
    public Page<CourseOnlineResponse> getAll(String keyword, String status, String trainerId, Pageable pageable) {
        CourseStatusOnline statusEnum = null;
        if (status != null && !status.isBlank()) {
            try {
                statusEnum = CourseStatusOnline.valueOf(status.toUpperCase());
        ...
        try (Workbook wb = new XSSFWorkbook(file.getInputStream())) {
            ...
            for (int i = 1; i <= sheet.getLastRowNum(); i++) {
                ...
                if (courseRepository.existsByCourseCode(code)) {
                    result.addError(rowNum, "courseCode", "CourseOnline code already exists: " + code);
                    continue;
                }
                ...
                if (levelStr != null && !levelStr.isBlank()) {
                    try {
                        course.setLevel(CourseLevelOnline.valueOf(levelStr.trim().toUpperCase()));
                    } catch (IllegalArgumentException ignored) {
                    }
                }
                ...
                if (statusStr != null && !statusStr.isBlank()) {
                    try {
                        course.setStatus(CourseStatusOnline.valueOf(statusStr.trim().toUpperCase()));
                    } catch (IllegalArgumentException ignored) {
                        course.setStatus(CourseStatusOnline.DRAFT);
                    }
                } else {
                    course.setStatus(CourseStatusOnline.DRAFT);
                }
