...
public class DepartmentController {
        ...
        public List<DepartmentDTO> getAll(
                ...
                return departmentService.getAll(status);
        }
        ...
        @Operation(summary = "Get department by ID", description = "Retrieve department details by UUID")
        @GetMapping("/{id}")
        public ResponseEntity<DepartmentDTO> getById(@PathVariable UUID id) {
                return ResponseEntity.ok(departmentService.getById(id));
        }

        @Operation(summary = "Update department", description = "Update department information by UUID")
        @PutMapping("/{id}")
        ...
        @Operation(summary = "Export departments", description = "Export department list to CSV or XLSX format")
        @GetMapping("/export")
        public ResponseEntity<byte[]> export(
                        @Parameter(description = "Export format: csv or xlsx", example = "xlsx") @RequestParam(defaultValue = "xlsx") String format) {
        ...
        @Operation(summary = "Import departments", description = "Upload Excel file (.xlsx) to import departments")
        @PostMapping(value = "/import", consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
        public ResponseEntity<ImportResultResponse> importDepartments(
                        @RequestPart("file") MultipartFile file) {
                ImportResultResponse result = importExportService.importDepartments(file);
                return ResponseEntity.ok(result);
        }

        @Operation(summary = "Download import template", description = "Download Excel template file for department import")
