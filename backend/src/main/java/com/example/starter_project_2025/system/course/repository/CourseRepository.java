...
public interface CourseRepository extends JpaRepository<Course, UUID> {
    ...
    @org.springframework.data.jpa.repository.Query("SELECT c FROM Course c WHERE " +
            "(:keyword IS NULL OR LOWER(c.courseName) LIKE LOWER(CONCAT('%', :keyword, '%')) OR LOWER(c.courseCode) LIKE LOWER(CONCAT('%', :keyword, '%'))) "
            +
            "AND (:status IS NULL OR c.status = :status) " +
            "AND (:trainerId IS NULL OR c.trainer.id = :trainerId)")
