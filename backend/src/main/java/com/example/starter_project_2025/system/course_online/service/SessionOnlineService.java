...
public class SessionOnlineService {
    ...
    public SessionOnlineResponse createSession(SessionOnlineRequest request) {
        ...
        CourseLessonOnline lesson = lessonRepository.findById(request.getLessonId())
                .orElseThrow(() -> new ResourceNotFoundException("Lesson", "id", request.getLessonId()));

        SessionOnline session = new SessionOnline();
    ...
    public SessionOnlineResponse updateSession(UUID id, SessionOnlineRequest request) {
        SessionOnline session = sessionRepository.findById(id)
                .orElseThrow(() -> new ResourceNotFoundException("SessionOnline", "id", id));

        mapRequestToEntity(request, session);

        if (request.getLessonId() != null && !request.getLessonId().equals(session.getLesson().getId())) {
            CourseLessonOnline newLesson = lessonRepository.findById(request.getLessonId())
                    .orElseThrow(() -> new ResourceNotFoundException("Lesson", "id", request.getLessonId()));
    ...
    public void deleteSession(UUID id) {
        if (!sessionRepository.existsById(id)) {
            throw new ResourceNotFoundException("SessionOnline", "id", id);
        }
        sessionRepository.deleteById(id);
    }

    public SessionOnlineResponse getSessionById(UUID id) {
        SessionOnline session = sessionRepository.findById(id)
                .orElseThrow(() -> new ResourceNotFoundException("SessionOnline", "id", id));
        return convertToResponse(session);
    }

    public List<SessionOnlineResponse> getSessionsByLesson(UUID lessonId) {
    ...
    private static final String[] SESSION_HEADERS = {
            "SessionOnline Order", "Topic", "Type", "Student Tasks", "Duration (minutes)"
    };

    public ResponseEntity<byte[]> exportSessions(UUID lessonId) {
        List<SessionOnline> sessions = sessionRepository.findByLessonIdOrderBySessionOrderAsc(lessonId);
    ...
    public ImportResultResponse importSessions(UUID lessonId, MultipartFile file) {
        ...
        CourseLessonOnline lesson = lessonRepository.findById(lessonId)
            ...
            while (rows.hasNext()) {
                ...
                try {
                    ...
                    if (typeVal != null && !typeVal.isBlank()) {
                        try {
                            session.setType(SessionTypeOnline.valueOf(typeVal.toUpperCase()));
    ...
    private void mapRequestToEntity(SessionOnlineRequest request, SessionOnline session) {
        ...
        if (request.getType() != null) {
            session.setType(SessionTypeOnline.valueOf(request.getType().toUpperCase()));
        }
    }

    private SessionOnlineResponse convertToResponse(SessionOnline session) {
        SessionOnlineResponse response = new SessionOnlineResponse();
